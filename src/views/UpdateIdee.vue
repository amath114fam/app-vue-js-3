...
const index = ideeStore.idees.findIndex(i => i.id === Number(route.params.id))

const nouveauTitre = ref(ideeStore.idees[index]?.titre || '')
const nouvelleCategorie = ref(ideeStore.idees[index]?.categorie || '')
const nouvelleDescription = ref(ideeStore.idees[index]?.description || '')
...
function updateIdee() {
  ...
  ideeStore.updateIdee(
    ideeStore.idees[index].id,
    nouveauTitre.value,
...
<template>
  <div class="formulaire">
    ...
    <button @click="updateIdee">Mettre à jour l'idée</button>
  </div>
</template>
